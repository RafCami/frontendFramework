 ...
 */
export const getAllMatchingPrograms = (campus: undefined | string, day: undefined | boolean, studyArea: undefined | string) => {
    return thomasMore.filter(p => programMatchesFilter(p, campus, day, studyArea))
}


const programMatchesFilter = (program: IProgram, campusId: string | undefined, day: boolean | undefined, studyAreaId: string | undefined) => {
    if (campusId && program.campus.id !== campusId) return false
    if (day !== undefined && program.day !== day) return false
